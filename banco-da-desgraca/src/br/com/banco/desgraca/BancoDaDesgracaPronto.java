...
public class BancoDaDesgracaPronto {
    ...
    public static void main(String[] args) throws Exception {
        ...
        ContaBancaria bbCc = new ContaCorrente(1L, BANCO_DO_BRASIL, 0.0);
        ContaBancaria bradescoCc = new ContaCorrente(2L, BRADESCO, 0.0);
        ContaBancaria caixaCc = new ContaCorrente(3L, CAIXA, 0.0);
        ContaBancaria itauCc = new ContaCorrente(4L,ITAU, 0.0);
        ContaBancaria nubankCc = new ContaCorrente(5L,NUBANK, 0.0);
        CONTAS_CORRENTE.addAll(asList(bbCc, bradescoCc, caixaCc, itauCc, nubankCc));

        ContaBancaria bbPoupanca = new ContaPoupanca(6L, BANCO_DO_BRASIL, 0.0);
        ContaBancaria bradescoPoupanca = new ContaPoupanca(7L, BRADESCO, 0.0);
        ContaBancaria caixaPoupanca = new ContaPoupanca(8L, CAIXA, 0.0);
        ContaBancaria itauPoupanca = new ContaPoupanca(9L,ITAU, 0.0);
        CONTAS_POUPANCA.addAll(asList(bbPoupanca, bradescoPoupanca, caixaPoupanca, itauPoupanca));

        ContaBancaria itauDigital = new ContaDigital(10L, ITAU, 0.0);
        ContaBancaria nubankDigital = new ContaDigital(11L, NUBANK, 0.0);
    ...
    private static void verificarContasQueNaoPodemSerCriadas() {
        ...
        contaInvalida(() -> new ContaDigital(21L, BANCO_DO_BRASIL, 150.0));
        contaInvalida(() -> new ContaDigital(22L, BRADESCO, 150.0));
        contaInvalida(() -> new ContaDigital(23L, CAIXA, 150.0));

        contaInvalida(() -> new ContaPoupanca(24L, NUBANK, 150.0));
    }
    ...
    private static void contaInvalida(Supplier<ContaBancaria> conta) {
        ...
        } catch (TipoContaNaoSuportadoException ibie) {
        }
    }
    ...
    private static void saldoInvalido(ContaBancaria conta, Double valor) {
        ...
        try {
//            System.out.println("Teste - " + conta.consultarSaldo());
            conta.sacar(valor);
            throw new RuntimeException("Atenção! Não deveria ser possível sacar " + getCurrencyInstance().format(valor) + " de " + conta);
